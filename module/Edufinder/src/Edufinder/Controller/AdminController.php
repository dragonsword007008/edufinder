...
use Edufinder\Form\UserForm;
use Edufinder\Form\UserFilter;
...
class AdminController extends AbstractActionController
{
    ...
    public function createAction()
    {
        ...
        if ($request->isPost()) {
			...
			if ($form->isValid()) {
				$data = $form->getData();
				unset($data['submit']);
				if (empty($data['registration_date'])) $data['registration_date'] = '2015-10-03 12:00:00';
				$this->getUsersTable()->insert($data);
				return $this->redirect()->toRoute('edufinder/default', array('controller' => 'admin', 'action' => 'index'));													
			}			
		}
    ...
    public function updateAction()
    {
		...
		if (!$id) return $this->redirect()->toRoute('edufinder/default', array('controller' => 'admin', 'action' => 'index'));
		$form = new UserForm();
		$request = $this->getRequest();
        if ($request->isPost()) {
			$form->setInputFilter(new UserFilter());
